...
export const createStyle = (
  ...
  if (!hasAnyProps) {
    return { classNames: [Styles.button], inlineStyles: {} };
  }

  // Build class names array for clsx
  const classNames: ClassValue[] = [Styles.button];
  ...
  // size
  addPropertyClasses(props.size, "size", classNames, toSnakeCase);

  // variant
  addPropertyClasses(props.variant, "variant", classNames, toSnakeCase);

  // disabled
  addPropertyClasses(props.disabled, "disabled", classNames);

  // fullWidth
  addPropertyClasses(props.fullWidth, "full_width", classNames);
